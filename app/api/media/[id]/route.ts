...
export async function GET(_req: Request, {params}: {params: Promise<{id:string}>}) {
  try {
    ...
    if (!asset || !(await publicOwnerActive(asset.owner_id))) throw new HttpError(404,"Image not found.");
    const path = `/api/media/${id}`;
    const {data:card,error:cardError} = await client.from("cards").select("id").eq("owner_id",asset.owner_id).eq("published",true).is("deleted_at",null).or(`published_data->>photo.eq.${path},published_data->>logo.eq.${path}`).limit(1).maybeSingle();
