...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{page?:string}>}) {
  ...
  return <><Header brand={activeBrand}/><main className="max-w-5xl mx-auto px-5 pb-16">
    <div className="flex flex-wrap justify-between items-start gap-4"><div><h1 className="text-3xl font-bold">My cards</h1><p className="cs-muted mt-2 text-sm">{user.email}</p></div><div className="flex gap-2"><Link href="/" className="cs-button cs-primary">Create card</Link><form action="/auth/signout" method="post"><button className="cs-button">Sign out</button></form></div></div>
